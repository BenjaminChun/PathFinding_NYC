  ...
  def search(self):
    ...
    while not open_set.empty():
      ...
      if self.number_of_steps>1:
        ...
        tempGraph.add_edge(current[2], current[3], global dist dict[(current[2], current[3])])

      # print(f"Selected Node {selected_node}")
      # check if the selected_node is the solution
      if current[2] == self.target: #comparing node to node
        path = self.calculate_path(selected_node) #need change calculate path
        return path, self.number_of_steps
      ...
      if len(new_nodes) > 0:
        for new_node in new_nodes:
          ...
          tempScore = current[0]+global distance dictionary[(current[2], new_node)]
          fuelLeft = current[4] - global fuel dict [(current[2], new_node)]
          if fuelLeft < 0: #should not take this path
            continue
          else:
            count+=1
            open_set.put((tempScore, count, new_node, fuelLeft))